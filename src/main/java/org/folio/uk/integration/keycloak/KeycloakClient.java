...
public interface KeycloakClient {
  ...
  TokenResponse login(@RequestBody MultiValueMap<String, String> loginRequest);

  @PostExchange(
    value = "/realms/{tenant}/protocol/openid-connect/token",
    contentType = APPLICATION_FORM_URLENCODED_VALUE,
    accept = APPLICATION_JSON_VALUE)
  TokenResponse login(@RequestBody MultiValueMap<String, String> loginRequest, @PathVariable("tenant") String tenant);

  @PostExchange(value = "/admin/realms/{realm}/users", contentType = APPLICATION_JSON_VALUE)
  ResponseEntity<Void> createUser(@PathVariable("realm") String realmName,
    @RequestBody KeycloakUser user,
    @RequestHeader(AUTHORIZATION) String token);

  @GetExchange(value = "/admin/realms/{realm}/users/{id}", accept = APPLICATION_JSON_VALUE)
  KeycloakUser getUser(@PathVariable("realm") String realmName,
    @PathVariable("id") String userId,
    @RequestHeader(AUTHORIZATION) String token);

  @GetExchange(value = "/admin/realms/{realm}/users?q={attrQuery}&briefRepresentation={brief}",
    accept = APPLICATION_JSON_VALUE)
  ...
  @PutExchange(value = "/admin/realms/{realm}/users/{id}", contentType = APPLICATION_JSON_VALUE)
  void updateUser(@PathVariable("realm") String realmName,
    @PathVariable("id") String userId,
    @RequestBody KeycloakUser user,
    @RequestHeader(AUTHORIZATION) String token);

  @DeleteExchange("/admin/realms/{realm}/users/{id}")
   ...
   */
  @GetExchange(value = "/admin/realms/{realm}/users?exact=true&first=0&max=1", accept = APPLICATION_JSON_VALUE)
   ...
   */
  @GetExchange("/admin/realms/{realm}/roles/{role}")
  ...
  @PostExchange("/admin/realms/{realmId}/clients/{clientId}/authz/resource-server/permission/scope")
  ...
  @DeleteExchange("/admin/realms/{realmId}/clients/{clientId}/authz/resource-server/permission/scope/{permissionId}")
   ...
   */
  @PostExchange("/admin/realms/{realm}/users/{userId}/federated-identity/{providerAlias}")
   ...
   * @param token - authorization token
   */
  @DeleteExchange("/admin/realms/{realm}/users/{userId}/federated-identity/{providerAlias}")
